// This example of Q learning and MRC controller
...
bool qlearning_control = true;// set this for using qlearning
bool post_marc_qlearning = true;// set this for using pure neural MRC for signal processing
...
//AND
//bool qlearning_control = false;
//bool post_marc_qlearning = false;
...
//        mrc_bias = 0.0;
      ...
      if(post_marc_qlearning)
      {
        if(post_mrc_output.at(0)>0.2) // left IR 0
        {
          e[1] = 1.0;
        }
        else
        ...
        if(post_mrc_output.at(1)>0.2) // Right IR 1
        {
          e[2] = 1.0;
        }
        else
      ...
      // e[1] = (int) sensors[5] ; //IR left
      //e[2] = (int) sensors[4]; //IR right

      /*
        e[0] = 1 other action, 0 stop;
      ...
      int j_RL = state; // convert e to a state id number. 0,...,7
      ...
      if(i_RL>=0) {
        ...
        //Simplify version
        //Q[a_RL][i_RL] = r_RL + discount_factor*Q[a1_RL][j_RL];
      }
