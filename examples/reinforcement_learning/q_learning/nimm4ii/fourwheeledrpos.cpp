...
namespace lpzrobots {
  ...
  int FourWheeledRPos::getSensorNumber(){
...
//      std::cout << 2 + irSensorBank.size() + conf.rpos_sensor_references.size() <<" Sensors total\n";
      return 2 + irSensorBank.size()+ conf.rpos_sensor_references.size()*3;
    }else{
//      std::cout << irSensorBank.size() << " ir Sensors, "<< conf.rpos_sensor_references.size() << " position sensors \n";
//      std::cout << 4 << " Wheel speed sensors \n";
//      std::cout << 4 + irSensorBank.size() + conf.rpos_sensor_references.size() <<" Sensors total \n";
      return 4 + irSensorBank.size() + conf.rpos_sensor_references.size()*3;
    }
 }

  int FourWheeledRPos::getSensors(sensor* sensors, int sensornumber){
    int len = 0;
   //if(sensornumber!= sensorno ){std::cout<<"warning: sensor number mismatch \n";}
    if(conf.twoWheelMode){
      ...
      len = Nimm4::getSensors(sensors,4);
    }
  ...
  int FourWheeledRPos::getMotorNumber(){
    if(conf.twoWheelMode)
      return 2;
    else 
  ...
  void FourWheeledRPos::setMotors(const motor* motors, int motornumber){
    if(conf.twoWheelMode){
      motor nimm4m[4];
      nimm4m[0] = motors[0];
      nimm4m[2] = motors[0];
  ...
  void FourWheeledRPos::doInternalStuff(GlobalData& globalData){
