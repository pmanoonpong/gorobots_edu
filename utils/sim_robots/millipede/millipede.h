...
namespace lpzrobots {
  ...
  struct MillipedeConf {
      ...
      /** distance from center of front legs */
      double legpos1;

      /** distance from center of back legs */
      double legpos2;
  ...
  struct millipedeSegment {
      Primitive* primitive;
    int segIndex;
    std::vector<int> legIndex;
    int nOfLegs;
    double height, width, length;
    osg::Matrix position;
    std::vector<double> legPosition;
    std::vector<double> legOrientation;
  ...
  class Millipede : public OdeRobot, public Inspectable {
    public:
       ...
       */
      static MillipedeConf getDefaultConf(double _scale = 1.0, int _legsPerSegment = 4, int _nOfSegments = 3, bool _useShoulder = 1, bool _useFoot = 1,
          bool _useBack = 0);

      static MillipedeConf getMillipedeConf(double _scale = 1.0, int _legsPerSegment = 4, int _nOfSegments = 3, bool _useShoulder = 1, bool _useFoot = 1,
          bool _useBack = 0);
       ...
       */
      Millipede(const OdeHandle& odeHandle, const OsgHandle& osgHandle, const MillipedeConf& conf = getDefaultConf(),
          const std::string& name = "Millipede robot");
       ...
       * leg.
       *
       * @param linkNum: link to be found (1 to n)
       *
       * Overload:
       * @param segmentNum: segment where the leg is placed (1 to n)
       * @param legNum: leg position number (1 to 4)
       * @param motNum: Motor on the leg1 (1 to 3)
       *
       *
       * @return the motor name value
       */
      int getMotorIndex(int linkNum);
      int getMotorIndex(int segmentNum, int legNum, int motNum);

      int getLegIndex(int segmentNum, int legNum);
...
//      static LegPos getMotorLegPos(MotorName);
    ...
    private:
      ...
      /** typedefs */
      typedef std::map<int, HingeJoint*> HingeJointMap;
      typedef std::map<int, Leg> LegMap;
      typedef std::map<int, ContactSensor*> LegContactMap;
      typedef std::map<int, OneAxisServo*> MotorMap;
      typedef std::map<int, TwoAxisServo*> LinkMap;
      typedef std::map<int, LegPosUsage> LegPosUsageMap;
      typedef std::map<int, IRSensor*> LegIRSensorMap;
      typedef std::vector<Primitive*> PrimitiveList;
      typedef std::vector<Joint*> JointList;
      typedef std::vector<TwoAxisJoint*> JointList2;
      typedef std::vector<OneAxisServo*> ServoList;
      typedef std::vector<millipedeSegment> SegmentList;
      ...
      // vector of segments to be created:
      ...
      // for two axis servos on the links
      LinkMap doubleServos;
      ...
      int getLinkIndex(int linkNum);


      int NumberOfLegs, NumberOfMotors, NumberOfSensors;
